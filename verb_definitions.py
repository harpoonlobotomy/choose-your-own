...
# {'and', 'with', 'to', 'the', 'at', 'plus', 'a'}, from, out, of,  "on", "against" ### Note: There may be more than one viable semantic when only one is required. That's fine.
...
verb_defs_dict = {
    "go to": {"alt_words":["go", "approach"], "allowed_null": None, "formats": [verb_loc]},
    ...
    "push": {"alt_words": ["shove", "move", "pull"], "allowed_null": None, "format": [verb_noun, verb_noun_dir, verb_noun_dir_noun]},
    "drop": {"alt_words": ["discard", ""], "allowed_null": None, "format": [verb_noun]},
    "open": {"alt_words": ["", ""], "allowed_null": None, "format": [verb_noun]},
    "read": {"alt_words": ["examine", ""], "allowed_null": None, "format": [verb_noun]}, ## Two nouns have 'examine'. Maybe make 'read' its own specific thing instead of referring 'examine' here. idk.
    "burn": {"alt_words": ["", ""], "allowed_null": None, "format": [verb_noun, verb_noun_sem_noun], "inventory_check": "fire_source"},
    "lock": {"alt_words": ["", ""], "allowed_null": None, "format": [verb_noun, verb_noun_sem_noun], "inventory_check": "key"},
    "unlock": {"alt_words": ["", ""], "allowed_null": None, "format": [verb_noun_sem_noun], "inventory_check": "key"},
    "open": {"alt_words": ["pry", ""], "allowed_null": None, "format": [verb_noun, verb_noun_sem_noun]},
    "break": {"alt_words": ["smash", ""], "allowed_null": None, "format": [verb_noun, verb_noun_sem_noun]},
    "take": {"alt_words": ["pick up", "get", "pick"], "allowed_null": None, "format": [verb_noun, verb_noun_sem_noun]}, # take ball, take ball from bag
    "put": {"alt_words": ["place", "leave"], "allowed_null": ["on", "in", "inside"], "format": [verb_noun_dir, verb_noun_sem_noun]}, # put paper down, put paper on table ## using 'leave' here might be tricky. But I want to allow for 'leave church' and 'leave pamphlet on table' both.
    "eat": {"alt_words": ["consume", "drink"], "allowed_null": None, "format": [verb_noun]},
    "look": {"alt_words": ["watch", "observe", "investigate", "examine"], "allowed_null": ["at", "to", "with"],  "format": [verb_only, verb_noun, verb_noun_sem_noun]}, # look, look at book, look at book with magnifying glass
    "set": {"alt_words": [""], "allowed_null": None, "format": [verb_noun_dir, verb_noun_sem_noun], "distinction": {"second_noun":"fire", "new_verb":"burn", "else_verb":"put"}} ## not implemented, just an idea. 'if fire is the second noun, the correct verb to use is 'burn', else the verb is 'put'. So 'set' is not its own thing, just a membrane/signpost.
    }
