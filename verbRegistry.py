...
class VerbInstance:
    ...
    def __init__(self, verb_key:str, attr:dict):
        ...
        self.null_words = attr["null_words"]
...
class VerbRegistry:
    ...
    def create_verb_instance(self, verb_key:str, attr:dict)->VerbInstance:
        ...
        for alt_words in attr["alt_words"]:
            if alt_words:
                if self.by_alt_words.get(alt_words):
                    print(f"Alt words {alt_words} already exists.")
                self.by_alt_words[alt_words] = inst
        if attr["requires_noun"]:
            self.requires_noun.append(inst)
        else:
            self.not_requires_noun.append(inst)
...
def initialise_registry():
    ...
    for item_name, attr in get_verb_defs().items():
