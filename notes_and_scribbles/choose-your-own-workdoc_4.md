...
12.37pm
I should have a set of sentence elements for the conversation engine. So it has a defined way of finding how the conversation topic is being used. ie:
    npc: I would talk to you about the rocks, but you wouldn't understand.
    potential expected sentence parts:
        '... about the rocks'
        'are the rocks ...'
        '{verb} the rocks to'
    etc. So that conversation elements have some kind of structure we can look for. Maybe not a clean defined list of items overall (ie any response to anything ever), but for each conversation piece, set which word structures would make sense. The same way I assign formats to each verb. It's manual labour for each new verb (and would be here for each conversation, though I'll make a dict of types I can just pull idx data from) but makes it so useful later. Will do the same for conversation elements. Right now, the conversation elements aren't parsed at all, as there's no real conversation - the npc will give a conversation, and the player can respond at certain points, but there's no actual conversing going on.

I could expand the parser to do the conversations too. It needs to be really, really cleaned up for that though. Right now it's so messy and I don't know which parts I can touch without breaking it all tbh. Needs a big redo. After I've done the save system and this one-shot test game. Then will redo the parser. I understand better what I need out of it now and can redo a whole bunch of it.

1.45pm
oh, I never gave npcs ids. Will fix.

I've added or removed a space somewhere - it now prints the input line again. Have added/removed a newline somewhere. Only after the autoplay ends, during autoplay it's fine. So I think that means I /added/ a printline after the input print?

I need to get rid of triggers for events that have finished. I can store that the item was a trigger some other way if I want, but having all triggers kept from past events is silly. Keep a specific 'history' note somewhere if needed down the line, but don't keep all triggers by default.

3.01pm
Will need to also get item.event from a current event's item as there is none in this autoplay. Just a note to self.
Working on adding event data to the savefile.

Will formalise a minimal 'prev. event' setup for events that don't need to keep any item/other data once they're done. Like the dried moss, that event has no use going forward (that I can see). Need a way to just have 'this event is done', in case that part's ever needed for something. Again it probably isn't.

okay so items need to save name + location always. at least I do right now - really I need a new init setup.
Currently my plan would be to add the default items, then change them based on metadata to match the 'updated' loaded versions. But, what if something moved? If the moss is by default in graveyard east, how would I know to assign the instance an id if it's now in the shed?

NOTE: Also need to add `flooring = place.placewide_surfaces["flooring"]` from itemReg to the savegames. Not yet but later maybe damagable tiles? I guess for now don't save, but later. Or at least the option.

4.52pm
this:
        if "container" in inst.item_type and not "electronics" in inst.item_type:
            if inst.print_children_as_list:
is the first instance of an item attr being required that is not saved in the savefile and is not autogenerated in the main route.

5.51pm
currently working on:

key in key_found: <ItemInst [wire cutters ID:54e8e6c9237b] [loc: north testing grounds] [event:None] >
item requires_key: [['78c13f46-8201-401e-9632-54e8e6c9237b']]
Exception: 'list' object has no attribute 'add'

and

about to clean relationships
Exception: 'str' object has no attribute 'name'

and

Exception: 'str' object has no attribute 'name'
done clean_relationships

All those are done.

6.10pm
Currently: loading seems to work, but local items are still added, but events are broken. So I can pick the iron key up and the map event works (though it shouldn't be there), but I can't unlock the gate.

'drop iron gate' incorrectly finds the item 'drop iron key'.
...
You don't have a iron to unlock the gate with.

it's correct here that I don't have 'iron key', but it should remember 'iron key' is a noun, because it's in encountered.

back to current debugging (just getting it running at all with the current scope of load data)

  File "d:\Git_Repos\choose-your-own\eventRegistry.py", line 82, in check_if_correct_key_exists_already
    if val.get(key.name) and isinstance(val[key.name], dict):
AttributeError: 'str' object has no attribute 'name'

Hmmm....

key in existing keys: iron key // val: {'iron key': {'key_is_placed_elsewhere': {'item_in_event': 'reveal_iron_key'}}}

So the issue is... that event might be over.
