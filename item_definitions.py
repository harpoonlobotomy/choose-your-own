...
item_defs_dict = {
    "scroll": {"name": "an old scroll", "description": f"An old parchment scroll, seemingly abandoned at a hidden shrine.", "flags": [CAN_PICKUP, CAN_OPEN, CAN_READ, CONTAINER, CLOSED], "item_size": SMALLER_THAN_BASKETBALL, "print_on_investigate": True, "starting_location": {"shrine": "north"}, "container_limits": SMALL_FLAT_THINGS, "starting_children": ["old gold key"],},
    "old gold key": {"name": "an old gold key", "description": f"A simple, sturdy looking gold key, worn with age but still in tact.", "flags": [CAN_PICKUP, IS_KEY, IS_HIDDEN], "item_size": SMALL_FLAT_THINGS, "starting_location": {"shrine": "north"}, "started_contained_in": "scroll"},
    "ivory pot": {"name": "an ivory-coloured jar", "description": f"A tall scratched ivory pot; no label, and a cork stopper held in place with a fine but strong-looking wire tied around the neck.", "description_no_children": f"A tall scratched ivory pot; no label, and a cork stopper held in place with a fine but strong-looking wire tied around the neck.", "flags": [CONTAINER, CAN_PICKUP, CAN_OPEN, CAN_LOCK, LOCKED, CLOSED], KEY:"wire cutters", "container_limits": SMALLER_THAN_APPLE, "item_size": SMALLER_THAN_BASKETBALL, "starting_location": {"shrine": "north"}},
    "metal pot": {"name": "a metal pot", "description": f"A stout metal pot, which would be called a pot if I wasn't testing how it deals with two jars; it seems to be dug into the earth, and stuck fast. It has a flat, plate-like lid, with a small gold lock holding it closed.", "flags": [CONTAINER, CAN_OPEN, CAN_LOCK, NEEDS_KEY_TO_LOCK, LOCKED, CLOSED], KEY:"old gold key", "container_limits": SMALLER_THAN_BASKETBALL, "item_size": BIGGER_THAN_BASKETBALL, "starting_location": {"shrine": "north"}, "starting_children": ["wire cutters"],},
    "wire cutters": {"name": "a pair of wire cutters", "description": f"A slightly rusted pair of wire cutters. They look like they'll still work, though.", "flags": [CAN_PICKUP, IS_KEY, IS_HIDDEN], "item_size": PALM_SIZED, "starting_location": {"shrine": "north"}, "started_contained_in": "metal pot"},
