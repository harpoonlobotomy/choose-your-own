...
def do_inventory():
    ...
    while not done:
        ...
        inv_test=option(game.inventory, print_all=True, none_possible=True, preamble="To examine an item more closely, type it here, otherwise hit 'enter' to continue.")
        #test = user_input()
        #print(f"Test: {test}")
        ...
        while True:
            if inv_test and inv_test in game.inventory:
                ...
                print()
                decision=option("drop", "separate", none_possible=True, preamble=f"You can drop the {inv_test} or try to separate it into parts, or hit enter to continue.")
                #text=user_input()
...
def user_input():
    ...
    if text.lower() == "help":
        print()
        slow_writing(f"  Type the (highlighted) words to progress, 'i' for 'inventory', 'd' to describe the environment, 'settings' to set some settings - that's about it.")
        print()
        return None
...
def option(*values, no_lookup=None, print_all:bool=False, none_possible:bool=True, preamble:str=None):
    ...
    while not option_chosen:
        ...
        if len(formatted) > 1:
            slow_writing(f"    {', '.join(formatted[:-1])} or {formatted[-1]}") # this formatting is good visually, so don't change it before  this. The delineation here is preferable.
        else:
...
def drop_loot(named=None, forced_drop=False):
    ...
    if named:
        ...
        loot_test = option(game.inventory, print_all=True, preamble="[Type the name of the object you want to leave behind]")
    ...
    if not game.checks["inventory_on"]:
        slow_writing("[[ Psst. Hey. Type 'secret' for a secret hidden option, otherwise type literally anything else. ]]")
        loot_test = user_input()
        if loot_test == "secret":
            ...
            #test = user_input()
            loot_test=option("yes", "no", preamble="Hey so... not everyone loves inventory management. So I'll give you the option now - do you want to turn it off and carry without limitation?")
            if loot_test in yes:
...
### drop random item if inventory full // not sure if I hate this. ###
    if len(game.inventory) > carryweight:
        print()
        heavy_test = option(f"drop", "ignore", preamble=f"It looks like you're already carrying too much. If you want to pick up {switch_the(item, 'the ')}, you might need to drop something - or you can try to carry it all.")
...
def relocate(need_sleep=None):
    ...
    if game.place==current_loc:
        ...
        slow_writing(f"You make your way to {game.place}. It's {game.time}, the weather is {game.weather}, and you're feeling {game.emotional_summary}.")
        if places[game.place].visited:
            slow_writing(f"You've been here before... It was {places[game.place].first_weather} the first time you came.")
...
def sleep_inside():
    slow_writing(f"Deciding to hunker down in {game.place} for the night, you find the comfiest spot you can and try to rest.")
...
def look_around(status=None):
    ...
    def look_dark():
        if ("torch" or "phone" or "matchstick") not in game.inventory:
            if descriptions[place].get("electricity"):
                look_test=option("yes", "no", preamble="It's dark, but you can try to find a light switch if you want.")
                if look_test in yes:
                    outcome = roll_risk()
                    if outcome in (1, 2):
            ...
            look_test=option("yes", "no", preamble=f"It's too dark to see much of anything in {switch_the(place, 'the ')}, and without a torch or some other light source, you might trip over something. Do you want to keep looking?")
            if look_test in yes:
                ...
                if outcome == 1:
                    ...
                    if len(game.inventory) < 4:
                        get_loot(1, random_bool=True, message=f"It's only a minor injury, sure, but damn it stings. You did find [item] while facefirst in the middle of [place], though.")
                    ...
                    if game.w_value:
                        print("You see something shimmer slightly off in a bush, but by the time you hobble over, whatever it was has vanished.")
                if outcome == 2:
                    hazard = get_hazard()
                    get_loot(1, random_bool=True, message=f"Narrowly avoiding tripping over {hazard}, you find [item]. Better than nothing, but probably all you'll find until there's more light.") # want options for the poorly lit hazard part.
                    # game.carrier_size compared to item.size? Can I be bothered?
                if outcome in (3,4):
                    get_loot(2, random_bool=True, message=f"Once your eyes adjust a bit, you manage to make out more shapes than you expected - you find [item].")
            if look_test in no:
                slow_writing("Thinking better of it, you decide to keep the advanced investigations until you have more light. What now, though?")
            look_test = option("stay", "go", preamble=f"You could stay and sleep in {switch_the(place, 'the ')} until morning, or go somewhere else to spend the wee hours. What do you do?")
            ...
            slow_writing("Using your torch, you see things. Probably. I haven't written this yet. How do you have a torch?!?")
    ...
    def look_light(reason=None):
        ...
        if not skip_intro: # if there are no other reason to skip intro, can just combine the above into one 'if/else'.
            slow_writing(f"Using the power of {reason}, you're able to look around {switch_the(game.place, 'the ')} without too much fear of a tragic demise.")
        ...
        if options:
            ...
            print(f"Interactable items: {options}") # Should stop it including the cardinals. Not sure why it was, but it was.
    ...
    if status == "exhausted" and current_time not in night:
        slow_writing("Stumbling, you're going to hurt yourself even if it's nice and bright. Haven't written it yet though.")
...
## ==== Actual start here ===
    ...
    if inside:
        ...
        add_text=""
    slow_writing(f"With the weather {add_text}{game.weather}, you decide to look around the {game.facing_direction} of {switch_the(game.place, 'the ')}.")
    if current_time in night:
        slow_writing(f"It's {game.time}, so it's {random.choice(choices.emphasis['low'])} dark.") # trying a bit more variation in the benign text.
        look_dark()
    else:
...
def describe_loc():
    ...
    while True:
        facing_direction=option(game.cardinals, "go", print_all=True, preamble="Pick a direction to investigate, or go elsewhere?")
...
def inner_loop():
    ...
    slow_writing(f"You wake up in {game.place}, right around {game.time}. You find have a bizarrely good sense of cardinal directions; how odd.")
    places[game.place].visited = True
    places[game.place].first_weather = game.weather

    slow_writing(f"`{game.playername}`, you think to yourself, `is it weird to be in {game.place} at {game.time}, while it's {game.weather}?`")
    print()
    describe_loc()

    test_innerloop=option("stay", "go", preamble="What do you want to do? (Stay) here and look around, or (Go) elsewhere?")
